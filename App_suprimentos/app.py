...
div[data-baseweb="select"] > div {
    background:#111827 !important; border:1px solid #334155 !important; border-radius:10px !important;
}
...
div[data-testid="stNumberInput"] button {
    color:#111827 !important; background:#E5E7EB !important; border-radius:8px !important;
}
.rank-card {
    background: rgba(255,255,255,0.04);
    border:1px solid rgba(255,255,255,0.08);
...
.small-note { color:#94A3B8 !important; font-size: 12px; }
</style>
""", unsafe_allow_html=True)

# =========================================================
# ARQUIVO FIXO DO REPOSITÓRIO
# =========================================================
...
def simular(base, saldo, tipos, familias, selecoes):
    ...
    for _, row in selecoes.iterrows():
        ...
        if filtro.empty or qtd_montar <= 0:
            continue
    ...
    consolidado["ORDEM_GRAFICO"] = consolidado.apply(
        lambda x: x["FALTA"] if x["FALTA"] > 0 else x["SOBRA"], axis=1
    )
...
def grafico_barras_tesla(df):
    ...
    fig.add_trace(go.Bar(
        ...
        orientation="h",
        name="Saldo",
        marker=dict(color=["#22C55E" if d >= 0 else "#EF4444" for d in plot_df["DELTA"]]),
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            size=12,
            color=["#EF4444" if v < 0 else "#10B981" for v in plot_df["DELTA"]],
            symbol="diamond"
        ),
...
def grafico_semanal(semanal):
    ...
    fig.add_trace(go.Bar(
        x=resumo["SEMANA"],
        y=resumo["NECESSIDADE"],
        name="Produção por semana",
        marker=dict(color="#38BDF8"),
        text=[formatar_compacto(x) for x in resumo["NECESSIDADE"]],
        textposition="outside",
        hovertemplate="<b>%{x}</b><br>Necessidade: %{y:,.2f}<extra></extra>",
    ))
    fig.update_layout(
        title="Simulação de Produção por Semana",
        xaxis_title="Semana",
        yaxis_title="Necessidade Total",
        paper_bgcolor="#0F1724",
        plot_bgcolor="#0F1724",
        font=dict(color="#E8EEF7"),
        margin=dict(l=10, r=20, t=60, b=10),
...
st.caption("Painel Tesla / PowerBI com ranking ampliado, cobertura de estoque e simulação por semana.")
...
try:
    ...
    st.caption("Para o mesmo COD. Modelo, informe quantidades diferentes por derivação e distribua por semana.")
    ...
    for idx, cod_modelo in enumerate(cod_modelos_sel, start=1):
        ...
        for j, deriv in enumerate(derivacoes_modelo, start=1):
            ...
            with cols[0]:
                st.text_input(f"Derivação {cod_modelo}-{j}", value=str(deriv), disabled=True, key=f"der_txt_{idx}_{j}")
            with cols[1]:
                semana = st.selectbox(f"Semana {cod_modelo}-{j}", semana_opcoes, index=0, key=f"sem_{idx}_{j}")
            with cols[2]:
                qtd = st.number_input(f"Qtd. {cod_modelo}-{j}", min_value=0, value=0, step=1, key=f"qtd_{idx}_{j}")
            with cols[3]:
                usar = st.checkbox("Selecionar", value=(qtd > 0), key=f"usar_{idx}_{j}")
    ...
    cobertura_semanas = 0 if total_nec == 0 else (total_saldo / max(total_nec / max(len(df_selecoes["SEMANA"].unique()), 1), 1))
    ...
    with t4:
        ...
        st.caption(f"Cobertura estimada: {cobertura_percentual:.1f}% | Cobertura semanal aprox.: {cobertura_semanas:.1f}")
    ...
    st.dataframe(df_selecoes, use_container_width=True, height=220)
    ...
    resumo = pd.DataFrame({
        "Indicador": ["Necessidade", "Saldo", "Delta", "Faltante", "Cobertura %", "Cobertura semanal aprox."],
        "Valor": [round(total_nec, 3), round(total_saldo, 3), round(total_delta, 3), round(total_falta, 3), round(cobertura_percentual, 2), round(cobertura_semanas, 2)]
    })
    ...
    st.download_button(
        ...
        data=arquivo,
        file_name="simulacao_suprimentos_tesla_v22.xlsx",
