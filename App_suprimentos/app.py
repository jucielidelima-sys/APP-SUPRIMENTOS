...
.der-card { background: rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:14px; padding:10px 12px; margin-bottom:10px; }
</style>
...
def carregar_excel_local_ou_upload():
    ...
    st.error("Arquivo padrão 'dados.xlsx' não encontrado no repositório.")
    st.stop()
...
def converter_excel_download(df_consolidado, df_detalhado, df_resumo, df_selecoes):
    output = BytesIO()
...
st.caption("Versão fixa no Git com dados carregados automaticamente e exportação Excel.")
...
try:
    ...
    detalhado, consolidado = simular(base, saldo, tipos_sel, familias_sel, df_selecoes)
    ...
    with t1:
        ...
        st.caption("Barra cinza: total necessário para atender as derivações selecionadas.")
    with t2:
        st.plotly_chart(termometro(total_saldo, max(total_nec, total_saldo), "Saldo", "#22C55E"), use_container_width=True)
        st.caption("Barra verde: saldo disponível para faturamento.")
    with t3:
    ...
    st.subheader("Tabela de seleções por modelo e derivação")
    ...
    arquivo = converter_excel_download(tabela, detalhado, resumo, df_selecoes)
    st.download_button(
        "Baixar simulação em Excel",
        data=arquivo,
        file_name="simulacao_suprimentos_tesla_v21.xlsx",
