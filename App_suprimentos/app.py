...
def grafico_barras_tesla(df):
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(12,16,22,0.72)",
        plot_bgcolor="rgba(12,16,22,0.52)",
...
def grafico_semanal_status(df):
    ...
    fig.add_trace(go.Bar(
        ...
        text=[formatar_compacto(v) for v in df["NECESSIDADE"]],
        textposition="outside",
        hovertemplate="<b>%{x}</b><br>Necessidade: %{y:,.0f}<extra></extra>",
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        text=[formatar_compacto(v) if v > 0 else "" for v in df["RUPTURA_SEMANA"]],
        textposition="outside",
        hovertemplate="<b>%{x}</b><br>Ruptura: %{y:,.0f}<extra></extra>",
    ))

    fig.add_trace(go.Scatter(
        x=df["SEMANA"],
        y=df["SALDO_APOS_SEMANA"],
        name="Saldo restante",
        ...
        hovertemplate="<b>%{x}</b><br>Saldo restante: %{y:,.0f}<extra></extra>",
    ))

    max_val = max(
        float(df["NECESSIDADE"].max()) if len(df) else 0,
        float(df["SALDO_APOS_SEMANA"].max()) if len(df) else 0,
        1
    )

    ticks = [0, max_val*0.25, max_val*0.50, max_val*0.75, max_val]
    ticktext = [fmt_axis(v) for v in ticks]

    fig.update_layout(
        title=dict(
            text="🏭 Simulação de Produção por Semana - Sala de Controle",
            y=0.97,
            x=0.02,
            xanchor="left",
            yanchor="top",
        ...
        plot_bgcolor="rgba(12,16,22,0.52)",
        font=dict(color="#E8EEF7"),
        margin=dict(l=10, r=20, t=110, b=10),
        height=600
    )
