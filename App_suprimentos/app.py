...
st.caption("Mantém o painel Tesla / PowerBI. Alterado somente o filtro de semanas para lista suspensa com 4 semanas.")
...
try:
    ...
    st.caption("Total de 4 semanas em lista suspensa. Você usa apenas as semanas necessárias.")

    semana_opcoes = ["Semana 1", "Semana 2", "Semana 3", "Semana 4"]
    linhas_selecao = []
    ...
    for idx, cod_modelo in enumerate(cod_modelos_sel, start=1):
        ...
        derivacoes_modelo = sorted(
            [x for x in subset_modelo["DERIVACAO"].unique().tolist() if str(x).strip() != ""],
            key=lambda x: str(x)
        )
        ...
        for j, deriv in enumerate(derivacoes_modelo, start=1):
            ...
            with cols[0]:
                st.text_input(
                    f"Derivação {cod_modelo}-{j}",
                    value=str(deriv),
                    disabled=True,
                    key=f"der_txt_{idx}_{j}"
                )

            with cols[1]:
                semana_sel = st.multiselect(
                    f"Semanas - {cod_modelo}-{j}",
                    semana_opcoes,
                    default=[],
                    key=f"semanas_{idx}_{j}"
                )

            with cols[2]:
                qtd_base = st.number_input(
                    f"Qtd base {cod_modelo}-{j}",
                    min_value=0,
                    value=0,
                    step=1,
                    key=f"qtdbase_{idx}_{j}"
                )

            with cols[3]:
                usar = st.checkbox(
                    "Selecionar",
                    value=(qtd_base > 0 and len(semana_sel) > 0),
                    key=f"usar_{idx}_{j}"
                )

            if usar and qtd_base > 0 and len(semana_sel) > 0:
                for sem in semana_sel:
                    linhas_selecao.append({
                        "COD_MODELO": cod_modelo,
                        "MODELO_DESC": desc_modelo,
                        "DERIVACAO": deriv,
                        "SEMANA": sem,
                        "QTD_A_MONTAR": qtd_base
                    })

    df_selecoes = pd.DataFrame(linhas_selecao)

    if df_selecoes.empty:
        st.warning("Selecione pelo menos uma derivação, uma ou mais semanas e quantidade maior que zero.")
        st.stop()
    ...
    st.download_button(
        ...
        data=arquivo,
        file_name="simulacao_suprimentos_tesla_v25.xlsx",
