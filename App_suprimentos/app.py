...
def preparar_saldo(df):
    saldo = pd.DataFrame({
        "DESCRICAO_COMPONENTE": df.iloc[:,0].apply(normalizar),
        "ITEM": df.iloc[:,2].apply(normalizar),
        "SALDO": df.iloc[:,3].apply(to_float),
    })
    saldo = saldo[saldo["ITEM"] != ""].copy()
    saldo = saldo.groupby("ITEM", as_index=False).agg(
...
def grafico_semanal_status(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df["SEMANA"], y=df["NECESSIDADE"], width=[0.5]*len(df), offsetgroup="n", name="Necessidade", marker=dict(color=cores), text=[formatar_compacto(v) for v in df["NECESSIDADE"]], textposition="outside"))
    fig.add_trace(go.Bar(x=df["SEMANA"], y=df["RUPTURA_SEMANA"], width=[0.32]*len(df), offsetgroup="r", name="Ruptura", marker=dict(color="#F97316"), text=[formatar_compacto(v) if v > 0 else "" for v in df["RUPTURA_SEMANA"]], textposition="outside"))
    fig.add_trace(go.Scatter(x=df["SEMANA"], y=df["SALDO_APOS_SEMANA"], name="Saldo restante", mode="lines+markers+text", text=[f"Saldo {formatar_compacto(v)}" for v in df["SALDO_APOS_SEMANA"]], textposition="top center", line=dict(color="#38BDF8", width=3), marker=dict(size=8, color="#38BDF8")))
    fig.update_layout(title="Simulação de Produção por Semana - Necessidade, Ruptura e Saldo", xaxis_title="Semana", yaxis_title="Quantidade", bargap=0.18, paper_bgcolor="#0F1724", plot_bgcolor="#0F1724", font=dict(color="#E8EEF7"), margin=dict(l=10, r=20, t=60, b=10), height=540, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0))
...
def mini_tabela(df):
    ...
    mini = base[["ITEM","DESCRICAO_COMPONENTE","NECESS_FMT","SALDO_FMT","DELTA_FMT","FALTA_FMT","STATUS"]].copy()
    mini.columns = ["ITEM","DESCRIÇÃO","NECESSIDADE","SALDO","DELTA","FALTA","STATUS"]
    return mini.head(10)
...
tabela = consolidado[["ITEM","DESCRICAO_COMPONENTE","NECESSIDADE","SALDO","DELTA","FALTA","SOBRA","STATUS"]].copy()
...
st.download_button("Baixar simulação em Excel", data=arquivo, file_name="simulacao_suprimentos_tesla_v32.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
